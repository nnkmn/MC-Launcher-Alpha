...
Minecraft 运行需要 Java 运行时环境（JRE 或 JDK）。MC Launcher Alpha 提供了自动检测和手动配置两种方式来管理 Java 环境。本章将详细介绍 Java 的检测、配置和内存调优方法。

## 自动检测

MC Launcher Alpha 会在首次启动时自动扫描系统中已安装的 Java 环境。
...
::: tip 提示
如果你的系统中安装了多个版本的 Java，MC Launcher Alpha 会列出所有检测到的版本，你可以从中选择要使用的版本。
:::
...
合理的内存分配对游戏性能至关重要。MC Launcher Alpha 允许你自定义 Java 的最大和最小内存分配。
...
- 你可以在系统中安装多个 Java 版本，并在 MC Launcher Alpha 中为不同实例指定不同的 Java 版本。
:::
